...
TEST_F(DatabaseHandlerTest, KO1_FR27_DatabaseWithNoData_InsertProfile) {
    this->database_handler.insert_or_update_charging_profile(1, ChargingProfile{1, 1});
...
TEST_F(DatabaseHandlerTest, KO1_FR27_DatabaseWithProfileData_UpdateProfile) {
    this->database_handler.insert_or_update_charging_profile(1, ChargingProfile{2, 1});
    this->database_handler.insert_or_update_charging_profile(1, ChargingProfile{2, 2});
...
TEST_F(DatabaseHandlerTest, KO1_FR27_DatabaseWithSingleProfileData_LoadsCharingProfile) {
    ...
    ASSERT_TRUE(sut.find(1) != sut.end());
...
TEST_F(DatabaseHandlerTest, KO1_FR27_DatabaseWithMultipleProfileDiffEvse_LoadsCharingProfile) {
    ...
    ASSERT_TRUE(sut.find(1) != sut.end());
    ASSERT_TRUE(sut.find(2) != sut.end());
    ASSERT_TRUE(sut.find(3) != sut.end());

    auto profiles1 = sut[1];
    auto profiles2 = sut[2];
    auto profiles3 = sut[3];

    ASSERT_EQ(profiles1.size(), 2);
    ASSERT_EQ(profiles2.size(), 2);
    ASSERT_EQ(profiles3.size(), 2);
}
