...
SmartChargingHandler::get_reported_profiles(const GetChargingProfilesRequest& request) const {
    ...
    auto matching_profiles =
        this->database_handler->get_charging_profiles_matching_criteria(request.evseId, request.chargingProfile);

    for (auto& [evse_id, profile, source] : matching_profiles) {
        profiles.push_back(ReportedChargingProfile(profile, evse_id, source));
    }
